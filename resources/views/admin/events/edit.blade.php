...
<x-layouts.admin-layout titulo='El Gran Danés Café - Bar | Admin | Crear evento'>
    ...
    <div class="events-form-container">
        ...
        <form action="{{ route('admin.events.update', $event->slug) }}" method="post" class="events-form" enctype="multipart/form-data">
            @csrf
            @method('put')
            ...
            <fieldset class="flex flex-col">
                ...
                @enderror
                <input type="date" name="date" id="event-date" value="{{ old('date') ?? $event->fecha }}">
            </fieldset>
            {{-- genero musical --}}
            <fieldset class="flex flex-col">
                ...
                @error('musical_genre')
                    <div class="text-red-500">{{ $message }}</div>
                @enderror
                <select name="musical_genre" id="musical_genre" value="{{ $event->genero }}">
                    <option value="" disabled selected></option>
                    <optgroup label="Populares">
                        <option {{ $event->genero === 'pop' ? 'selected' : '' }} value="pop">Pop</option>
